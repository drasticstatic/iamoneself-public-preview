...
export default function FAQPage() {
  ...
  // Highlight matching text in FAQ answers when searching
  function highlightText(text: string): { __html: string } {
    if (!lc.trim()) return { __html: text };
    // Escape regex special chars
    const escaped = lc.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
    const regex = new RegExp(`(${escaped})`, "gi");
    const highlighted = text.replace(regex, '<mark class="bg-amber-200/70 dark:bg-amber-800/40 text-amber-900 dark:text-amber-200 rounded-sm px-0.5">$1</mark>');
      ...
      <section className="relative px-6 py-24 mx-auto max-w-4xl text-center">
        ...
        <div className="mt-8 relative max-w-md mx-auto">
          ...
          <button
            ...
            className={`absolute right-3 top-1/2 -translate-y-1/2 flex items-center justify-center rounded-full transition-colors ${showExamples ? "text-amber-500 bg-amber-50 dark:bg-amber-950/30" : "text-neutral-400 hover:text-amber-500"}`}
      ...
      <section className="px-6 pb-24 mx-auto max-w-4xl space-y-16">
        {filtered.map((category) => {
          ...
          const Icon = meta?.icon || Lightbulb;
            return (
              <div key={category.category} id={slug(category.category)} className="flex flex-col items-center">
